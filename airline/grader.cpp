...
static bool used[MAX_V*(MAX_V-1)/2];
static bool exist_edge[MAX_N][MAX_N];
static bool used_edge[MAX_N][MAX_N];
static int cnt_makemap;
static bool aliceturn = true;
void init(){
	V = 0;
	U = 0;
	N_ = 0;
...
static long long parse(long long a, long long b, long long c){
	return a * 100000000LL + b * 10000LL + c;
}

void WrongAnswer( int e ){
  fprintf(stdout, "%lld", parse(127382876, e, e));
  exit( 0 );
}
...
void InitG( int in_V, int in_U ){
	if(not aliceturn) WrongAnswer(18);
...
int main( int argc, char** argv ){	
   ...
   printf("%lld", parse(756688401, V, N));
}
